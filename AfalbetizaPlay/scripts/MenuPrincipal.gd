...
## confirmar visualmente que o app sabe pra quem salvar os pontos.
func _atualizar_label_jogador() -> void:
	if PerfilJogador.nome_atual.is_empty():
		_label_jogador.text = ""
	else:
		_label_jogador.text = "Jogando como: %s (%s)" % [PerfilJogador.nome_atual, PerfilJogador.turma_atual]
...
func _on_botao_ranking_pressed() -> void:
	get_tree().change_scene_to_file("res://scenes/ui/TelaRanking.tscn")


func _on_botao_trocar_jogador_pressed() -> void:
	get_tree().change_scene_to_file("res://scenes/ui/TelaNomeJogador.tscn")
